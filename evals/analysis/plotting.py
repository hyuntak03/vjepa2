...
def plot_layer_val_acc(heads, val_acc, out_path, subtitle=None):
    ...
    # group (layer, acc) by probe label ; probe label = name after the first '_'
    series = defaultdict(list)
    for h in heads:
        probe_label = h["name"].split("_", 1)[1] if "_" in h["name"] else h["name"]
    ...
    for probe_label in sorted(series):
        ...
        # annotate best layer per probe
        bx, by = max(pts, key=lambda t: t[1])
        ax.annotate(f"L{bx}:{by:.1f}", (bx, by), textcoords="offset points", xytext=(0, 6), fontsize=8)
