...
"""RollOut_v2 — 학습셋 자 (attentive) 로 읽은 p / z / h 위치의 요약 그림 두 장.
  ...
  figures/summary/fig_motion_gain_scatter.png  gain 의 원본: 시나리오·축별 (진실 변위, 읽은 변위) 산점도 + 회귀 직선 (p, h). 기울기의 부호·크기와 점 퍼짐(상관)을 한눈에.
  figures/summary/fig_motion_gain.png  시나리오·축별 motion gain = clip 단위 (읽은 슬롯 0→7 변위) 를 (진실 변위) 에 회귀한 기울기. 1 = 진실만큼, 0 = 안 움직임. 막대 위 숫자 = gain (2026-09-17 까지는 상관이었다 — 막대 값으로 오독됨). 상관은 stdout 에 찍는다.
...
def main():
    ...
    NOPROP = ["flat_v", "flat_a", "flat_d", "arc", "fall"]; PROP = ["ramp_a", "ramp_d", "ledge", "wall"]; order = NOPROP + PROP
    oi = [SCEN.index(s) for s in order]; xs = np.arange(len(order)); LAB = {"flat_v": "flat\nconst. v", "flat_a": "flat\nconst. a", "flat_d": "flat\ndecel.", "arc": "arc", "fall": "fall", "ramp_a": "ramp", "ramp_d": "ramp\nuphill decel.", "ledge": "ledge", "wall": "wall"}
    ...
    # ── fig 2: x(t), y(t) 위치-시간. 원점 = 그 clip 의 진실 슬롯 0 위치 (모든 곡선 공통, 출발 오차가 보인다). 좌상단 = (t 0, 변위 0), + 는 아래 (2026-09-17 사용자 요청) ──
    fig, axes = plt.subplots(2, len(SCEN), figsize=(7.0 * 2, 5.4), sharex=True)
    t = np.arange(T)
    for c, s in enumerate(SCEN):
        k = np.where((sc == s) & (pl == 1))[0]
        # x 는 진행 방향으로 정렬 (좌/우 clip 이 평균에서 상쇄되지 않게): 진실 x 변위 부호를 곱한다. 정지 clip 은 +1
        # 진행 방향 = 문맥 구간의 운동 방향 (wall 은 미래에서 되튕기므로 미래 부호로 정하면 pass 가 뒤집힌다)
        sgn = np.sign(Lall[k][:, 7, 0] - Lall[k][:, 0, 0]); s7 = np.sign(truth[k][:, 7, 0] - truth[k][:, 0, 0]); sgn[sgn == 0] = s7[sgn == 0]; sgn[sgn == 0] = 1; SG = np.stack([sgn, np.ones_like(sgn)], -1)[:, None, :]   # (n,1,2)
        for a, ax in enumerate(axes[:, c]):
            ...
            for r in [r for r in ("p", "z", "h") if r in P]:
                d = (pred[r][k][:, :, a] - truth[k][:, :1, a]) * SG[:, :, a]; ax.plot(t, d.mean(0), color=COL[r], lw=1.6, label=r)
            ...
            lo, hi = ax.get_ylim(); ax.set_ylim(max(hi, 1.0), min(lo, -0.05 * max(hi, 1.0))); ax.set_xlim(0, T - 1)   # 0 을 위에, + 를 아래로: 좌상단이 원점
            if c == 0:
                ax.set_ylabel(("x from true start (px)\nalong travel, + = down" if a == 0 else "y from true start (px)\n+ = down"))
            if a == 0:
                ax.set_title(s)
            else:
                ax.set_xlabel("future tubelet"); ax.set_xticks(range(0, T, 2))
    h, l = axes[0, -1].get_legend_handles_labels()
    fig.legend(h, l, loc="lower center", ncol=6, frameon=False, fontsize=12, handlelength=3.0, columnspacing=2.5, bbox_to_anchor=(0.5, 0.0))
    fig.text(0.5, 0.10, "origin = the clip's true position at future tubelet 0 (top-left). mean over clips, band = ±1 std. x is signed along the context travel direction; both axes grow downward, so a readout behind the truth sits above the black line.", ha="center", fontsize=7.5)
    fig.tight_layout(rect=(0, 0.13, 1, 1)); fig.savefig(FIG / "fig_motion_xy.png", dpi=170); plt.close(fig)
    ...
    for a, ax in enumerate(axes):
        ...
        for j, r in enumerate([r for r in ("p", "z", "h") if r in P]):
            ...
            for s in rows:
                ...
                print(f"gain {'xy'[a]} {s:8s} {r}  slope {g[-1]:5.2f}  corr {cc[-1]:5.2f}  n {int(k.sum())}")
            ax.bar(xs + (j - 1) * w, g, w, color=COL[r], label=r)
            for x_, g_, c_ in zip(xs, g, cc):
                ax.text(x_ + (j - 1) * w, max(g_, 0) + 0.03, f"{g_:.2f}", ha="center", fontsize=5.5, color=COL[r])
    ...
    fig.text(0.5, -0.02, "bars = per-clip regression slope of read vs true slot 0→7 displacement (1 = reproduces the true motion); number = the same slope. Axes with no true motion (flat y, fall x, wall) omitted.", ha="center", fontsize=6.5)
    fig.tight_layout(); fig.savefig(FIG / "fig_motion_gain.png", dpi=200, bbox_inches="tight"); plt.close(fig)

    # ── fig 3b: gain scatter — 시나리오 하나 = 상자 하나 (왼쪽 Δx, 오른쪽 Δy). 점 = clip, 가로 = 진실이 슬롯 0→7 동안 움직인 거리, 세로 = 읽기가 움직인 거리 (2026-09-17) ──
    nr = (len(SCEN) + 1) // 2                                  # 2026-09-19: 시나리오 수에 맞춰 행 수 (7 -> 4행, 9 -> 5행)
    fig = plt.figure(figsize=(10.5, 3.6 * nr + 0.1)); sub = fig.subfigures(nr, 2, wspace=0.06, hspace=0.10).ravel()
    for si, s in enumerate(SCEN):
        sf = sub[si]; sf.suptitle(s, fontsize=12, fontweight="bold"); axs = sf.subplots(1, 2); sf.subplots_adjust(wspace=0.45, top=0.85, bottom=0.14)
        k = (sc == s) & (pl == 1)
        for a, ax in enumerate(axs):
            dt = truth[k][:, 7, a] - truth[k][:, 0, a]; lim = max(np.abs(dt).max(), 20) * 1.2
            for r in [r for r in ("p", "h") if r in P]:
                dr = pred[r][k][:, 7, a] - pred[r][k][:, 0, a]; lim = max(lim, np.abs(dr).max() * 1.1)
            xx = np.array([-lim, lim]); ax.plot(xx, xx, color="k", lw=0.6, ls=":", label="read = truth"); ax.axhline(0, color="#bbb", lw=0.4); ax.axvline(0, color="#bbb", lw=0.4)
            fit = dt.std() > 2
            for r in [r for r in ("p", "h") if r in P]:
                dr = pred[r][k][:, 7, a] - pred[r][k][:, 0, a]
                ax.scatter(dt, dr, s=5, color=COL[r], alpha=0.25, lw=0, rasterized=True)
                if fit:
                    g, b = np.polyfit(dt, dr, 1); cc = np.corrcoef(dt, dr)[0, 1]
                    ax.plot(xx, g * xx + b, color=COL[r], lw=1.8, label=f"{r}  gain {g:.2f} (corr {cc:.2f})")
                else:
                    ax.plot([], [], color=COL[r], lw=1.8, label=f"{r}  (truth static, no fit)")
            ax.set_xlim(-lim, lim); ax.set_ylim(-lim, lim); ax.set_aspect("equal")
            ax.set_xlabel(f"true \u0394{'xy'[a]} (px)", fontsize=8.5); ax.set_ylabel(f"read \u0394{'xy'[a]} (px)", fontsize=8.5)
            ax.legend(fontsize=6.5, frameon=False, loc="upper left"); ax.spines[["top", "right"]].set_visible(False); ax.tick_params(labelsize=7)
    for sf in sub[len(SCEN):]:
        sf.set_visible(False)
    fig.text(0.5, 0.012, "horizontal = how far the object really moved from future slot 0 to 7 (px); vertical = how far the readout moved. left panel of each scenario: x axis, right panel: y axis.\n"
                         "one point = one possible clip. dotted: readout moves exactly as the truth (gain 1). solid: least-squares fit, its slope is the gain bar in fig_motion_gain; corr = how tightly points sit on it. z omitted (\u2248 h).", ha="center", fontsize=8)
    fig.savefig(FIG / "fig_motion_gain_scatter.png", dpi=150, bbox_inches="tight"); plt.close(fig)
    print(f"→ {FIG}/fig_l2.png, fig_motion_xy.png, fig_motion_gain.png, fig_motion_gain_scatter.png")
