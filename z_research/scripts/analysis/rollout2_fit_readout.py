#!/usr/bin/env python3
"""RollOut_v2_training (v5, 무중력 등속; ROLLOUT2_TRAIN) 의 p 로 위치 readout w 를 정하고 저장한다. test 는 하지 않는다.
...
저장: z_research/RollOutV2/exp_results/<train>/spatial_pooling/p/fit/{w_p.npy, fit.json}
...
import rollout2_test_readout as rt                         # noqa: E402  (학습셋 경로는 여기서: ROLLOUT2_TRAIN=v5)
ROOT = rt.ROOT; CACHE = rt.TR_CACHE; INDEX = rt.TR_INDEX
OUT = rt.RES_ROOT / "spatial_pooling/p/fit"
...
def main():
    ...
    key = "visible_by_sample" if "visible_by_sample" in idx[0] else "in_frame_by_sample"       # v5 부터: 화면 밖·가려진 튜블릿은 라벨에서 뺀다
    inf = (np.stack([arr(r[key]) for r in idx]) > 0).reshape(n, 16, 2).all(2)[:, T:]
    scen = np.array([r["scenario"] for r in idx]); print(f"visible 미래 튜블릿 {inf.sum()} / {inf.size}")
    ...
    mm = inf.ravel(); Xf = X.reshape(-1, D).astype(np.float64)[mm]; Yf = L.reshape(-1, 2)[mm]
