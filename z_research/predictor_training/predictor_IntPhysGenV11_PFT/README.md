...
- ⚠️ 정정 (09-17): v11 은 속도 1 종·궤적 고정이라 held-out 도 학습 궤적의 재생이다. v11 결과 = **학습 궤적 재생**, 상태 이어가기 증거 아님; post-FT p 가 문맥에서 무엇을 읽는지는 미측정 (정체성 probe 안 걸음). 상태 이어가기 검정은 속도가 다양한 데이터로 학습 후 RollOut β→1 여부 (다음 실험).
