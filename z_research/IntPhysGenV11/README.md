...
곁가지 3: [`../RollOutV2/`](../RollOutV2/README.md) — **위치 readout: p 가 물체를 어디에 두나** (2026-09-12 완료, 정본 `figures/v5/summary/POSITION_READOUT_2026-09-12.md`).
별도 학습셋 자 (held-out 0.87 칸) 로 p / z / h 를 읽는다. encoder 는 어디서나 물체를 읽고 (≤ 0.6 칸), p 는 등속만 물체 위 (0.71 칸); ledge 는 선반 높이 유지 (부유), wall 은 2 슬롯 통과 뒤 물체 소실;
v11 은 물체다운 토큰이 남는 길이 = late 0 < early/mid 3 < 가림막 없음 4~5 슬롯 (자 없는 검사와 일치). **채점 (early 97.8 / late 57.5) 과 위치는 별개 축.**
⚠️ 자는 물체가 없어도 기본값 위치를 낸다 — "마지막 자리에 둔다" 는 철회됐다. 위치 주장은 슬롯별 attention 질량과 함께만.
...
| 2 | ~~`p` 문맥 잔상 검정 (위치 디코딩)~~ ✅ 2026-09-12 `../RollOutV2/` (곁가지 3) | 3 | 캐시만 |
